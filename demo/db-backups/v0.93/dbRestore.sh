...
download_and_unzip(){
  wget -O $DEST_LOCATION/$OPENELIS_SQL_FILE $GITHUB_BASE_URL/$OPENELIS_SQL_FILE.gz
  wget -O $DEST_LOCATION/$ODOO_SQL_FILE $GITHUB_BASE_URL/$ODOO_SQL_FILE.gz
  wget -O $DEST_LOCATION/$OPENMRS_SQL_FILE $GITHUB_BASE_URL/$OPENMRS_SQL_FILE.gz
  wget -O $DEST_LOCATION/$BAHMNIPACS_SQL_FILE $GITHUB_BASE_URL/$BAHMNIPACS_SQL_FILE.gz
  wget -O $DEST_LOCATION/$PACSDB_SQL_FILE $GITHUB_BASE_URL/$PACSDB_SQL_FILE.gz
...
restore(){
  ...
  psql -U$PSQLUSER $OPENELIS_DB_NAME < $DEST_LOCATION/$OPENELIS_SQL_FILE
  ...
  psql -U$PSQLUSER $ODOO_DB_NAME < $DEST_LOCATION/$ODOO_SQL_FILE
  ...
  psql -U$PSQLUSER $PACS_DB_NAME < $DEST_LOCATION/$PACSDB_SQL_FILE
